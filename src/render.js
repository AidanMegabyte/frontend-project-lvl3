        ...
        <div class="row">
          <div class="col">
            <div class="form-floating">
              <input type="text" class="form-control" required autofocus id="${rssUrlInputId}" name="${rssUrlFormData}" placeholder="Ссылка RSS">
              <label for="${rssUrlInputId}">Ссылка RSS</label>
            </div>
          </div>
          <button id="${rssAddButtonId}" type="submit" class="col-auto btn btn-primary btn-lg px-sm-5">Добавить</button>
        </div>
      </form>
...
// Отрисовка фидов RSS
const renderRssFeeds = (feeds) => renderRssPart(feedListContainerId, 'Фиды', feeds, 'feed');

// Отрисовка постов RSS
const renderRssPosts = (posts) => renderRssPart(postListContainerId, 'Посты', posts, 'post');
